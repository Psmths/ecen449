...
int main(){
	...
	while(1){
		...
		//7   6   5   4     3  2  1  0
		_Bool b0 = ( 0x01 & XGpioDiscreteRead(&sws,1) == 0x01); //Button 1 status 0001 0x1 INC_COUNT
		_Bool b1 = ( 0x02 & XGpioDiscreteRead(&sws,1) == 0x02); //Button 2 status 0010 0x2 DEC_COUNT
		_Bool b2 = ( 0x04 & XGpioDiscreteRead(&sws,1) == 0x04); //Button 3 status 0100 0x4 DISP_SWTC
		_Bool b3 = ( 0x08 & XGpioDiscreteRead(&sws,1) == 0x08); //Button 4 status 1000 0x8 DISP_CNT
		...
		if(b2){
			switches = (0xF0 & XGpio_DiscreteRead(&swb,1)) >> 4; //Get switch mask (high nib) and shift
